...
Rules:
- Identify decision type from the QUESTION.
- Extract gt_decision and pred_decision.
- decision_match=1 only if decisions clearly match.
- If unsure, use "unknown" and decision_match=0.
...
def ollama_error_tag(
    ...
    pred_decision: str | None,
    decision_match: int | None,
    model: str,
    url: str,
    timeout_s: int,
    retries: int,
...
""".strip()
    ...
    if len(summary.split()) > 30:  # hard guard
...
def main():
    ap = argparse.ArgumentParser(description="Score SoccerChat preds JSONL with token_f1, rougeL_f1, and judges")
    ap.add_argument("--input", default="SoccerChat_valid_xfoul_abs_preds_100.jsonl")
    ap.add_argument("--output", default="viz/SoccerChat_valid_xfoul_abs_preds_100_scored.jsonl")
    ap.add_argument("--group", default="valid_preds_100")
    ap.add_argument("--ollama_judge", action="store_true")
    ...
    ap.add_argument("--judge_url", default="http://localhost:11434/api/generate")
    ap.add_argument("--judge_timeout_s", type=int, default=45)
    ...
    with inp.open("r", encoding="utf-8") as f, out.open("w", encoding="utf-8") as w:
        for line in f:
            ...
            # fail_score: decision mismatch dominates
